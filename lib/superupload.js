 ...
 * init(options)
 *
 * Prepares a psuedo-object (hash) with properties for each of our handler functions. Takes an options
 * hash as an argument with two settings: uploadDir and maxUploadSize
 *
 * Example:
 *
 *  var superupload = require("superupload.js).init({
    ...
    exports.upload = function (req, res) {
        ...
        form.parse(req, function(err, fields, files) {
            if (err) {
                ...
                res.write( "<a href='/files/" + file.path + "'>Uploaded to here</a>" );
            } else {
                res.writeHead( 400, {'content-type': 'text/plain'} );
                res.write( "No fileId" );
            }
